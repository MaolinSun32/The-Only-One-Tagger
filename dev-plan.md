...
- **`TagMerger`**（`src/management/tag-merger.ts`）：将标签 A 合并到标签 B，或从全库中删除标签 A（删除模式）。启动前调用 `OperationLock.acquire("标签合并")`，获取失败时 Notice 提示并拒绝启动；完成后 `OperationLock.release()`。与 Schema Editor 的"同步更新"共用 `BulkYamlModifier` 基类（`src/management/bulk-yaml-modifier.ts`），提供逐文件追踪 + 崩溃恢复能力。
  ...
  3. 用户确认后执行（**持久化 merge log + Registry 写入后置**）：
     ...
     - **StagingStore 同步清理**：扫描 `tag-staging.json`，合并模式 → 将所有笔记中 label 为 A 的条目替换为 B（保留 `user_status`、`badge` 等状态不变，仅改 label）；删除模式 → 将所有笔记中 label 为 A 的条目移除。防止用户 applyAll 残留的旧标签时撤销合并/删除结果
...
*文档版本：12.0 | 日期：2026-03-17 | 状态：第八轮架构审核完成（4 项修正：批量处理增加 max_batch_size 上限默认 50、TagMerger 增加删除模式、RegistryStore 增加 deleteTag() 方法、TagMerger 增加 StagingStore 同步清理步骤防止 applyAll 撤销合并结果），8 模块开发计划就绪*
