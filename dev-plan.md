...
**步骤 2：识别 tags**
- 输入：该 type 的 facet 定义 + 各 facet 对应的标签库候选子集（硬编码过滤，全量传入，不截断）+ 笔记全文
- 输出：`{ facet: [tags] }` 映射
...
> **§2.3 概念模型与 M5 实现的映射**：§2.3 的三步概念模型是高层总览，M5 AnalysisOrchestrator 将其展开为 7 个实现步骤。步骤 1（识别 type）= Orchestrator step 1；步骤 2（识别 tags）= Orchestrator steps 2-5（PromptFilterBuilder → generateTags → AIResponseValidator → TagMatcher）；步骤 3（本地组装）= Orchestrator steps 6-7（StagingStore 写入 + VerificationPipeline）。
...
- 已通过 applyAll 写入 registry 的标签（`verified_by: manual`）：registry 中标记 `flagged: true`；标签浏览器（M8）提供"待复核标签"筛选器；侧边栏中被 flagged 的标签显示 ⚠️ 图标；Notice 通知可点击跳转到标签浏览器定位该标签。用户可选择：修正拼写（触发全库替换）、确认保留（取消 flagged）、删除
...
- **`PromptAssembler`**（`src/ai/prompt-assembler.ts`）：组装两步 AI 调用的 prompt 文本。**依赖 PromptFilterBuilder（M3）提供 taxonomy 候选 + SchemaResolver（M3）提供 facet 定义/enum values**：
...
- **`AIResponseValidator`**（`src/ai/ai-response-validator.ts`）：校验 AI 步骤 2 返回的 `{ facet: [tags] }` 映射，防止非法输出污染 staging。位于 PromptAssembler（构建请求）和 AnalysisOrchestrator（消费结果）之间。校验规则：
  ...
  3. **Taxonomy 黑名单解析**：经过规范化的 taxonomy 标签 → 查 `RegistryStore.getBlacklistMap(facets)`：命中 rejected 标签 → 自动替换为 `rejected_in_favor_of` 目标标签。同时区分 verified（🟢 库内标签）和新词
  ...
  - 依赖 SchemaResolver（M3）获取 facet 定义/enum values/enum blacklist，RegistryStore（M2）获取 taxonomy 黑名单映射
...
- **`AnalysisOrchestrator`**（`src/operations/analysis-orchestrator.ts`）：编排单篇笔记的完整分析流程，是"分析当前笔记"命令的核心实现。**入口处对 schema 做 deep clone 快照**，整个分析流程使用快照而非实时引用，确保单篇分析内的 schema 一致性（防止分析期间用户修改 Schema Editor 导致中途 facet 定义变化）。提供两个入口方法：
  - **`analyzeNote(file: TFile)`**：完整流程（含 type 检测），执行步骤 1-7
  - **`analyzeWithType(file: TFile, type: string)`**：跳过 type 检测，直接以给定 type 执行步骤 2-7。供 `TypeOperationExecutor` 的 `addType`/`changeType` 调用

  **步骤详情**：
  1. 调用 `GenerationProvider.detectType()` → 获得 type（`analyzeWithType` 跳过此步）
  2. 调用 `PromptFilterBuilder.build(type)` → 获得候选标签子集（全量，不含黑名单）
  3. 调用 `GenerationProvider.generateTags(type, candidates, note)` → 获得 `facet→tags` 映射
  4. **AIResponseValidator 校验**：过滤非法 facet、规范化 taxonomy 值、**硬编码解析 taxonomy 黑名单和 enum 黑名单**（`resolveBlacklist()`）、处理单值/多值不匹配
  5. 本地组装：validator 输出中已区分 verified（🟢 库内标签，含黑名单解析结果）和新词
...
- **`TagOperationExecutor`**（`src/operations/tag-operation-executor.ts`）：纯业务逻辑，不涉及 UI。**所有 registry 写入（入库、黑名单）统一推迟到 `applyAll`**，逐条操作仅修改 staging 状态：
  ...
  - `applyAll(notePath)` → 收集该笔记所有 `user_status: "accepted"` 的标签，按**"先写最危险的、再写安全的"**顺序执行，确保中途失败时可安全重试：
    1. **Facet 有效性校验**：用当前 schema 校验 staging 中的 facet——已从 schema 中删除的 facet 跳过写入，通过 Notice 通知用户
    2. **构建 `TagWriteData`**（纯内存计算）：从 accepted 标签中提取本次涉及的 type 列表和各 type 的 facet→tags 映射
    3. **第一步：写入笔记 YAML**（最可能失败——涉及文件系统 I/O）。调用 `FrontmatterService.write(file, tagWriteData)` 执行增量合并写入。`write()` 内部读取现有 YAML → 将新 type 追加到 `type` 数组 → 合并 type 块 → 保留不在本次写入中的现有 type 块 → `_tag_version` 递增。**失败则直接停止，不执行后续步骤，用户可安全重试**
    4. **第二步：写入 registry**（内存 + JSON 写入，失败概率低）：🟢 registry badge → registry 不变；🔵/🟡 新 badge → `RegistryStore.addTag()`（`verified_by` 取决于 badge，**幂等**）；标签已有但当前 facet 不在其 `facets[]` 中 → `RegistryStore.expandFacets()` 自动追加；处理 `replaces` 链 → `RegistryStore.rejectTag()`（**幂等**）
...
- **Tab B：标签模式（Schema Editor）**，两级可展开列表：
  ...
  - **Schema 修改/删除的同步策略**：
    ...
    - **崩溃恢复**："同步更新"涉及全库 YAML 批量修改，采用与 TagMerger（M8）相同的恢复机制——通过 `BulkYamlModifier` 共用基类，将进度持久化到 `schema-sync-state.json`（逐文件追踪 `pending_files`/`completed_files`），Obsidian 中断后可从上次进度恢复。同步期间显示进度 UI
    - "仅修改模式"：只改 schema，不动已有 YAML 和 registry
  - **批量处理期间锁定**：当 `BatchProcessor` 正在运行时，Tab B 顶部显示"⚠️ 批量处理运行中，请暂停后再修改模式"，所有编辑控件禁用
...
- Schema Editor 批量锁定：batch 运行时编辑控件禁用，暂停后恢复
- Schema 同步：修改/删除时弹窗正确显示受影响笔记，"同步更新"正确修改 YAML + registry + staging，"仅修改模式"只改 schema
- Schema 同步崩溃恢复：同步 150/300 文件后模拟中断 → 重启后 `schema-sync-state.json` 正确恢复 → 剩余文件继续处理
...
- **`TagMerger`**（`src/management/tag-merger.ts`）：将标签 A 合并到标签 B。与 Schema Editor 的"同步更新"共用 `BulkYamlModifier` 基类（`src/management/bulk-yaml-modifier.ts`），提供逐文件追踪 + 崩溃恢复能力。
  1. **Dry-run 预览**：扫描全库 YAML，列出所有包含 A 的笔记及将发生的修改
  2. **Git 提示**：检测 vault 是否为 git 仓库（`.git` 目录存在），合并前提示建议 commit
  3. 用户确认后执行（**持久化 merge log + Registry 写入后置**）：
     - `BackupManager.createBackup("tag-registry.json")` 创建备份
     - 创建 `merge-state.json`：记录 `source_tag`、`target_tag`、`pending_files`（待处理文件列表）、`completed_files`（已处理）、`status: "running"`
     - 逐文件通过 `FrontmatterService` 将 A 替换为 B，**每成功修改一个文件立即将其从 `pending_files` 移到 `completed_files` 并持久化**
     - **所有 YAML 修改完成后**才写入 Registry：A → `rejected_in_favor_of: B`，B 继承 A 的 relations（确保中断时 registry 状态与"未完成合并"一致）
     - `merge-state.json` 标记为 `status: "completed"`
  4. **启动恢复**：插件启动时检测 `merge-state.json`（`status: "running"`）→ 提示用户是否继续执行剩余 `pending_files` 的合并
...
步骤 2 → AI 返回候选 taxonomy 标签
      ...
      │     └── 未命中 → 新词
      │
      ├── 🟢 库内标签 → 进入 staging，展示给用户
      │     → 用户 Accept → staging 标记 accepted
      │     → 用户 Delete → staging 标记 deleted
      │
      └── 新词 → badge 初始化为 ⚪ verifying → 进入 staging
              │     操作按钮禁用
              │
              ├── Wikipedia 确认     → badge 更新为 🔵 wiki_verified，按钮启用
              ├── Search+AI 确认    → badge 更新为 🔵 search_verified，按钮启用
              └── 都未确认/未配置    → badge 更新为 🟡 needs_review，按钮启用
              │
              └── 展示给用户，等待操作（✓/✗ 为三态切换，可随时撤回）：
                  │
