...
- `src/types.ts`：所有接口定义（含 `StagingTagItem.ai_recommended`）
...
- **`TagOperationExecutor`**（`src/operations/tag-operation-executor.ts`）：纯业务逻辑，不涉及 UI。**所有 registry 写入（入库、黑名单）统一推迟到 `applyAll`**，逐条操作仅修改 staging 状态：
  ...
  - `applyAll(notePath)` → 检查 `OperationLock.isLocked()`，被占用时 Notice 提示并拒绝执行。未占用时，收集该笔记标签，按**"先写最危险的、再写安全的"**顺序执行，确保中途失败时可安全重试：
    ...
    2. **构建 `TagWriteData`**（纯内存计算）：收集 `user_status: "accepted"` 的标签 + `user_status: "pending"` 且 `ai_recommended: false` 的标签（原 YAML 已有、用户未操作、默认保留），作为该 facet 的**完整值集合**。`deleted` 标签不被收集 = 不写入 = 从 YAML 移除
...
- **Tab B：标签模式（Schema Editor）**，两级可展开列表：
  ...
  - **Schema 修改/删除的同步策略**：
    ...
    - "同步更新"：启动前调用 `OperationLock.acquire("Schema 同步")`，获取失败时 Notice 提示并拒绝启动；完成后 `OperationLock.release()`。**YAML + Registry + Staging 同时更新**（如重命名 facet 时，YAML 键名、registry 中标签的 `facets` 数组、staging 中的 facet 键名一起改），执行前自动 `BackupManager` 备份 registry + 提示用户建议 git commit
...
- **`TagMerger`**（`src/management/tag-merger.ts`）：将标签 A 合并到标签 B。启动前调用 `OperationLock.acquire("标签合并")`，获取失败时 Notice 提示并拒绝启动；完成后 `OperationLock.release()`。与 Schema Editor 的"同步更新"共用 `BulkYamlModifier` 基类（`src/management/bulk-yaml-modifier.ts`），提供逐文件追踪 + 崩溃恢复能力。
...
*文档版本：10.0 | 日期：2026-03-17 | 状态：第七轮架构审核完成（4 项修正：合并语义全量替换 + ai_recommended 标签分流、已有标签审查式 prompt 设计、DataStore 错误隔离、OperationLock 全局互斥），8 模块开发计划就绪*
