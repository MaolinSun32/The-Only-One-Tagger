...
| `StagingStore` | DataStore 子类 + 7 业务方法 | staging 数据的统一操作入口（写入/更新/清理/全局搜索） | M4 VerificationPipeline, M5 Executors, M7 BatchProcessor, M8 TagMerger |
...
*文档版本：14.0 | 日期：2026-03-18 | 状态：第十轮架构审核完成（3 项修正：手动模式 staging 初始化改为按需加载 YAML 完整集合+StagingStore 增加 addTagToFacet 方法、AIResponseValidator 集成 TagMatcher 实现 aliases 匹配、TagMerger staging 清理增加合并去重防止同 facet 重复 label），8 模块开发计划就绪*
