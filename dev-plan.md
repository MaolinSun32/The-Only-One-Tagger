...
| `content_hash` | 分析时笔记内容的 SHA-256 前 8 位。用户打开审核时重新计算哈希并比对，不匹配则提示"笔记内容已变更，建议重新分析" |
...
- **`VerificationQueueManager`**（`src/verification/verification-queue-manager.ts`）：管理 `verification-queue.json`。离线时将待验证标签入队；监听 NetworkMonitor 的 `online` 事件，自动批量重试。记录 `attempts` 计数，超过阈值标记为 `needs_review`。**验证完成后同时检查 RegistryStore 和 StagingStore 两个位置**：标签已在 registry 中则更新 `verified_by`；标签还在 staging 中则更新 `badge`（如 `needs_review` → `wiki_verified`）。
...
- **`AnalysisOrchestrator`**（`src/operations/analysis-orchestrator.ts`）：编排单篇笔记的完整分析流程，是"分析当前笔记"命令的核心实现：
  ...
  5. 全部结果写入 `StagingStore`：库内标签 badge 为 `registry`（🟢），新词 badge 为 `verifying`（⚪）
  6. 新词并发走 `VerificationPipeline` → badge 从 ⚪ 异步更新为 🔵 / 🟡（UI 通过事件订阅实时刷新，按钮从禁用变为启用）
...
- **`TagOperationExecutor`**（`src/operations/tag-operation-executor.ts`）：纯业务逻辑，不涉及 UI。**所有 registry 写入（入库、黑名单）统一推迟到 `applyAll`**，逐条操作仅修改 staging 状态：
  ...
  - `applyAll(notePath)` → 收集该笔记所有 `user_status: "accepted"` 的标签，**统一执行所有 registry 操作**：
    ...
    5. `FrontmatterService.write()` 写入 YAML（增量合并，保留不在 staging 中的现有标签）
...
*文档版本：6.0 | 日期：2026-03-12 | 状态：第三轮架构审核完成（13 项修订），8 模块开发计划就绪*
